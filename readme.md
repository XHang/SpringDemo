...
1. 往自己的项目的pom文件添加以下依赖
   ...
   目的：在于从spring-boot-starter-parent继承过来，添加的依赖大多无需添加版本
       <dependency>
                   <groupId>org.springframework.boot</groupId>
   ...
   目的：我们这个程序的目的就是测试Spring Data JPA，所以加这个spring-boot-starter-test的启动器
...
2. 添加关于数据库的配置
   在resources文件夹里面创建一个文件application.properties ,里面填充一下内容
   ...
   才怪，BUG记录贴
   1. 启动时报Method org.postgresql.jdbc.PgConnection.createClob() is not yet implemented
      但是此错误抛出后，程序继续运行。
      这是一个JBOSS社区广为人知的BUG,它存在于Spring-Boot 2.0.0.RC1之前或者更高的版本中
      需要在application.properties文件追加此配置
...
JPA全称为Java Persistence API.,它通过注解或者XMl把bean和数据库的表关联起来，并可以将这个bean保存到数据库中。
...
一般Spring Data JPA的配置都是写在resources文件夹里面的application.properties
...
2. 在外部调用的service方法上面写上@Transactional注解，不过这需要你加上下面的配置
...
4. 可以手动使用编程来执行事务，关于这个的其他信息，可以参考本项目里面的streamQuery的方法
...
Spring Data jpa支持两种不同Criteria API的实现，

第一种是用原始的EntityManager来实现。虽然要写很多的样板代码，但是可以书写复杂的查询。

第二种用你的Repository类继承自JpaSpecificationExecutor类，你的repository类就有了几个findAll的方法

老实说吧，Spring Data Jpa 提供的Criteria APi虽然减少了样板代码的书写，但其实它只能支持where级别的查询，并且只能查询当前的实体类。

为什么这么说，你看到toPredicate返回值是什么，Predicate对象，这个是一个谓词，表达的意思类似于x!=y

Spring data jpa拿来Predicate对象是塞在CriteriaQuery的where里面的。
...
Criteria API包含三个几个核心类

CriteriaQuery你就把它看做是整个sql语句行了，它上面有N多方法可以用，像什么groupby 啊，where 啊，select 啊
...
其实Criteria api的东西大致就是这样了，更多内容，还可以看看我写的例子。
...
Querydsl这个是一个框架，可以构建静态类型的sql查询
...
1. 默认情况下，是根据对象的主键字段来检查的，如果对象的主键字段是null的，save对象就执行插入操作。
   否则，就是更新操作。
2. 当然你可以自定义实现，让你的域类(实体类)去实现Persistable ，覆盖里面的isNew方法。
   然后你知道怎么做吧。。
3. 其他少用，不讲
...
1. 现在数据库有两张表，两张表里面有很多个字段都是一样的。现在你要为这两张表构建实体类。
   ...
   然后分别建子类，放独有的字段。
   所以步骤是这样的
   1. 建立抽象类，抽象类要加@MappedSuperclass注解
...
这个时候，就要用一个东西了@ManyToMany和@JoinTable注解

在老师类和学生类建一个集合字段，表示一个老师就可以教多名学生，一个学生可以被多名老师教

然后在集合字段上（假设是老师实体类的学生集合字段）加

    @ManyToMany(fetch = FetchType.LAZY)
    @JoinTable(name = "std_tea",joinColumns =@JoinColumn(name =  "tea_id"),inverseJoinColumns =@JoinColumn( name ="std_id"))
    

解释下

JoinTable 的name是关系表的名字

JoinColumn name是所在表对应关系表的外键字段。

inverseJoinColumns 是所在对应表对应关系表的外键字段

嘛，其实看了解释也不会有多懂，看看实际的注解值就饿OK了
